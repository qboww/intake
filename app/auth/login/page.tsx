        ...
        <div className="flex flex-col items-center gap-8">
          ...
          {error && (
            <div className="w-full p-4 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg">
              <p className="text-sm font-medium text-red-800 dark:text-red-200">
                {error === 'Callback' && 'Email address not whitelisted'}
                {error === 'AccessDenied' && 'Access denied'}
                {!['Callback', 'AccessDenied'].includes(error) && 'An error occurred during sign in'}
              </p>
            </div>
          )}
          ...
          <div className="w-full space-y-4">
            <button
              ...
              disabled={isLoading}
              className="w-full flex items-center justify-center gap-3 px-4 py-3 border border-gray-300 dark:border-gray-700 rounded-lg font-medium text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-900 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
            >
              {isLoading ? (
                <>
